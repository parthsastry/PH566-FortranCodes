program ques4
    ...
    write(*,1) rdf(1,1), rdf(1,2), rdf(1,3)
    write(*,*) "Numerical values of RDF for r = 1.0 - "
    write(*,1) rdf(2,1), rdf(2,2), rdf(2,3)
    write(*,*) "Numerical values of RDF for r = 2.5 - "
    write(*,1) rdf(3,1), rdf(3,2), rdf(3,3)
endprogram
...
subroutine two_s(r, out)
    real :: r,out
    out = (1.0/sqrt(32.0))*(2 - r)*exp(-r/2.0)
    RETURN
end

subroutine two_p(r, out)
    real :: r,out
    out = (1.0/sqrt(972.0))*(6 - 6*r + r**2)*exp(-r/2.0)
    RETURN
end
